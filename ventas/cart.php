...
$grand_total = 0;
$select_cart = $connect->prepare("SELECT carro.idv, usuarios.id, usuarios.nombre, productos.precio,productos.stock,productos.foto,productos.idprod, productos.codpro, productos.nomprd, carro.name, carro.price, carro.quantity FROM carro INNER JOIN usuarios ON carro.user_id = usuarios.id INNER JOIN productos ON carro.idprod = productos.idprod;");
 $select_cart->execute();
